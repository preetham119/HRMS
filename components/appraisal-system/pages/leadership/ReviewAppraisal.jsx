...
export default function LeadershipReviewAppraisal() {
  const { id } = useParams();
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['leadership-review', id],
    queryFn: () => appraisalApi.leadershipReview(id).then((r) => r.data.data),
  });

  useEffect(() => {
    if (!data?.leadershipReview) return;
    const lr = data.leadershipReview;
    setForm({
      comments: lr.comments || '',
      development_plan: lr.development_plan || '',
      strengths: lr.strengths || '',
      improvements: lr.improvements || '',
      recommend_promotion: !!lr.recommend_promotion,
      promotion_type: lr.promotion_type || '',
      recommend_increment: !!lr.recommend_increment,
      salary_revision: lr.salary_revision || '',
      role_change: lr.role_change || 'No Change',
      recommended_training: lr.recommended_training || '',
      high_potential: !!lr.high_potential,
      final_rating: lr.final_rating != null ? String(lr.final_rating) : (data.hrReview?.final_rating != null ? String(data.hrReview.final_rating) : ''),
    });
  }, [data]);

  const locked = data?.appraisal?.status !== 'under_leadership_review'
    || data?.leadershipReview?.status === 'submitted';

  const mutation = useMutation({
    mutationFn: (payload) => appraisalApi.saveLeadershipReview(id, payload),
    onSuccess: (res) => {
      toast.success(res.data.message);
      setConfirm(false);
      qc.invalidateQueries({ queryKey: ['leadership-review', id] });
      qc.invalidateQueries({ queryKey: ['leadership-queue'] });
      qc.invalidateQueries({ queryKey: ['leadership-dashboard'] });
      qc.invalidateQueries({ queryKey: ['notifications'] });
      if (res.data.message?.toLowerCase().includes('submitted')) {
        navigate('/leadership/approvals');
      }
    },
  ...
  const adminAvg = Number(data.adminReview?.final_rating ?? data.hrReview?.final_rating) || 0;
  const overall = averageRating([selfAvg, mgrAvg, adminAvg, Number(form.final_rating)].filter((v) => v > 0));
  const summaryChart = [
    { name: 'Employee', score: selfAvg },
    { name: 'Manager', score: mgrAvg },
    { name: 'Admin', score: adminAvg },
  ].filter((x) => x.score > 0);
  ...
  return (
    <PageTransition>
      <PageHeader
        ...
        breadcrumbs={[
          { label: 'Leadership', to: '/leadership/dashboard' },
          { label: 'Approvals', to: '/leadership/approvals' },
      ...
      <TabPanel value={tab} index={2}>
        <Grid container spacing={2.5}>
          <Grid item xs={12} md={7}>
            <Card>
              <CardContent>
                <Typography variant="h6" fontWeight={700} mb={2}>Admin Review</Typography>
                {!(data.adminReview || data.hrReview) ? (
                  <Alert severity="info">Admin review not available</Alert>
                ) : (
                  <Stack spacing={2}>
                    <Typography variant="body2" color="text.secondary">
                      By {(data.adminReview || data.hrReview).reviewerId || 'Admin'} · {(data.adminReview || data.hrReview).status}
                    </Typography>
                    <StarScore value={adminAvg} showBadge />
                    <Typography variant="subtitle2">Comments</Typography>
                    <Typography variant="body2">{(data.adminReview || data.hrReview).comments || '—'}</Typography>
                    <Typography variant="subtitle2">Development Plan</Typography>
                    <Typography variant="body2">{(data.adminReview || data.hrReview).development_plan || '—'}</Typography>
                    <Typography variant="subtitle2">Training</Typography>
                    <Typography variant="body2">{(data.adminReview || data.hrReview).recommended_training || '—'}</Typography>
                  </Stack>
                )}
              </CardContent>
            </Card>
          ...
          <Grid item xs={12} md={5}>
            <Card>
              <CardContent>
                <Typography variant="h6" fontWeight={700} mb={2}>Review Summary</Typography>
                <Stack spacing={1.25}>
                  <Stack direction="row" justifyContent="space-between"><Typography variant="body2">Employee</Typography><Typography fontWeight={700}>{selfAvg || '—'}</Typography></Stack>
                  <Stack direction="row" justifyContent="space-between"><Typography variant="body2">Manager</Typography><Typography fontWeight={700}>{mgrAvg || '—'}</Typography></Stack>
                  <Stack direction="row" justifyContent="space-between"><Typography variant="body2">Admin</Typography><Typography fontWeight={700}>{adminAvg || '—'}</Typography></Stack>
                  <Divider />
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Typography fontWeight={700}>Overall</Typography>
                    <Chip label={ratingLabel(overall)} color="success" size="small" />
                  </Stack>
                </Stack>
              </CardContent>
            </Card>
      ...
      <TabPanel value={tab} index={3}>
        <Grid container spacing={2.5}>
          <Grid item xs={12} md={8}>
            <Card>
              <CardContent>
                <Typography variant="h6" fontWeight={700} mb={2}>Leadership Comments & Recommendations</Typography>
                <Grid container spacing={2} alignItems="flex-start">
                  <Grid item xs={12}>
                    <TextField
                      label="Overall Leadership Comments"
            ...
            <Card sx={{ position: { md: 'sticky' }, top: { md: 88 } }}>
              <CardContent>
                ...
                <Stack spacing={1.25}>
                  ...
                  {locked && <Alert severity="success">Leadership review finalized</Alert>}
                </Stack>
              </CardContent>
            </Card>
          </Grid>
      ...
      <ConfirmDialog
        open={confirm}
        title="Submit Leadership Approval?"
        message="This will finalize the appraisal as completed. Continue?"
        confirmText="Submit"
