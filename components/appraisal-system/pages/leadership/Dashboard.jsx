...
export default function LeadershipDashboard() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => dashboardApi.leadership().then((r) => r.data.data),
  ...
  return (
    <PageTransition>
      <PageHeader
        title="Leadership performance overview"
        subtitle="Leadership review and recommendations overview"
        breadcrumbs={[{ label: 'Leadership' }, { label: 'Dashboard' }]}
      />

      <DashboardProfileBanner profile={d.profileBanner} />

      <WorkflowProgressBar progress={d.workflowProgress} role="leadership" title="Your Progress" />

      <Grid container spacing={2.5} mb={3}>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Pending Approvals" value={d.pendingCount || 0} icon={<PendingActionsIcon />} color="warning" />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatCard title="Finalized" value={d.completedCount || 0} icon={<DoneAllIcon />} color="success" />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
      ...
      <Card>
        <CardContent>
          <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography variant="h6" fontWeight={700}>Awaiting Leadership Review</Typography>
            <Button size="small" onClick={() => navigate('/leadership/approvals')}>View all</Button>
          </Stack>
          <DataTable
            rows={d.queue || []}
            emptyTitle="No pending leadership reviews"
            emptyDescription="Appraisals reviewed by Admin will appear here."
            columns={[
              ...
              },
              {
                ...
                label: 'Action',
                render: (row) => (
                  <Button size="small" variant="contained" onClick={() => navigate(`/leadership/review/${row.id}`)}>
                    Review
                  </Button>
                ),
              },
